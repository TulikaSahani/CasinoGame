...
public class rewardanimation : MonoBehaviour
{
    ...
    public void Rewardcoin(int no_coin)
    {
        ...
        for (int i = 0; i < CoinParents.transform.childCount; i++)
        {
            CoinParents.transform.GetChild(i).DOScale(0.5f, 0.3f).SetDelay(delay).SetEase(Ease.OutBack);
            CoinParents.transform.GetChild(i).GetComponent<RectTransform>().DOAnchorPos(new Vector2 (60f , -330f ), 1f).SetDelay(delay + 0.5f).SetEase(
                Ease.OutBack);
            CoinParents.transform.GetChild(i).DORotate(Vector3.zero , 0.5f).SetDelay(delay + 0.5f).SetEase(Ease.Flash);
            CoinParents.transform.GetChild(i).DOScale(0.0f, 0.3f).SetDelay(delay +1f).SetEase(Ease.OutBack);
            delay += 0.1f;
        }
    }
